...
export const V3ActiveSummonerGrid = ({
  ...
  const removeAllButton =
    activeSummonerCount > 0 ? (
      <button
        type="button"
        disabled={isDisabled}
        onClick={(e) => {
          e.stopPropagation();
          if (!isDisabled) onClearAll();
        }}
        title={v3Locales.activeGrid.removeAllButton}
        className="flex items-center gap-1 px-2 py-1 rounded text-[10px] font-display uppercase tracking-wider text-red-400/80 hover:text-red-300 hover:bg-red-500/10 border border-red-500/20 hover:border-red-400/40 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
      >
              ...
              <div key={`active-row-${rowIndex}`} className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  {teamOneSummoner ? (
                    ...
                    <div
                      onDragOver={(event) => {
                        if (effectiveDragDisabled) return;
